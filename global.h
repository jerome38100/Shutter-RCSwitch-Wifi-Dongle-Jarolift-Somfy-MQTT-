...
#define ACCESS_POINT_NAME      "Shtter-RC-Dongle"  // default SSID for Admin-Mode
...
unsigned short jaroliftcnt = 0x0;                      // Initial 16Bit countervalue, stored in EEPROM and incremented once every time a command is send
int jaroliftadr = 110;                                 // EEPROM address where the 16Bit counter is stored.
unsigned short somfycnt = 0x0; 
int somfyadr = 2150;
...
boolean debug_log_radio_receive_all = true;
...
struct strConfig {
  ...
  String new_serial = "";
  String new_jaroliftcounter = "";
  String new_somfycounter = "";
  boolean set_and_generate_serial = false;
  boolean set_jaroliftcounter = false;
  boolean set_somfycounter = false;
} config;
...
void WriteConfig()
{
  ...
  WriteStringToEEPROM(1350, config.channel_name[16]);
  WriteStringToEEPROM(1400, config.channel_name[17]);
  WriteStringToEEPROM(1450, config.channel_name[18]);
  WriteStringToEEPROM(1500, config.channel_name[19]);
  WriteStringToEEPROM(1550, config.channel_name[20]);
  WriteStringToEEPROM(1600, config.channel_name[21]);
  WriteStringToEEPROM(1650, config.channel_name[22]);
  WriteStringToEEPROM(1700, config.channel_name[23]);
  WriteStringToEEPROM(1750, config.channel_name[24]);
  WriteStringToEEPROM(1800, config.channel_name[25]);
  WriteStringToEEPROM(1850, config.channel_name[26]);
  WriteStringToEEPROM(1900, config.channel_name[27]);
  WriteStringToEEPROM(1950, config.channel_name[28]);
  WriteStringToEEPROM(2000, config.channel_name[29]);
  WriteStringToEEPROM(2050, config.channel_name[30]);
  WriteStringToEEPROM(2100, config.channel_name[31]);
  EEPROM.commit();
  delay(1000);
} // void WriteConfig
...
boolean ReadConfig()
{
  ...
  if (config.cfgVersion <= 2) // read config parts up to version 2
  {
    ...
    config.channel_name[16] = ReadStringFromEEPROM(1350, 25);
    config.channel_name[17] = ReadStringFromEEPROM(1400, 25);
    config.channel_name[18] = ReadStringFromEEPROM(1450, 25);
    config.channel_name[19] = ReadStringFromEEPROM(1500, 25);
    config.channel_name[20] = ReadStringFromEEPROM(1550, 25);
    config.channel_name[21] = ReadStringFromEEPROM(1600, 25);
    config.channel_name[22] = ReadStringFromEEPROM(1650, 25);
    config.channel_name[23] = ReadStringFromEEPROM(1700, 25);
    config.channel_name[24] = ReadStringFromEEPROM(1750, 25);
    config.channel_name[25] = ReadStringFromEEPROM(1800, 25);
    config.channel_name[26] = ReadStringFromEEPROM(1850, 25);
    config.channel_name[27] = ReadStringFromEEPROM(1900, 25);
    config.channel_name[28] = ReadStringFromEEPROM(1950, 25);
    config.channel_name[29] = ReadStringFromEEPROM(2000, 25);
    config.channel_name[30] = ReadStringFromEEPROM(2050, 25);
    config.channel_name[31] = ReadStringFromEEPROM(2100, 25);
  }
  ...
  { // upgrade config to version 2
    config.mqtt_devicetopic = "shutterrc"; // default devicetopic
    config.cfgVersion = 2;
    // clear EEPROM space
...
void InitializeConfigData()
{
  ...
  if (!ReadConfig())
  {
    ...
    config.mqtt_broker_client_id = "ShutterRCDongle";
    config.mqtt_broker_client_id += chipIdString;
    config.mqtt_devicetopic = "shutterrc";
  ...
  if (config.mqtt_broker_client_id == "Shutter-RC-Dongle") {
    config.mqtt_broker_client_id += chipIdString;
